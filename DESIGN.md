# Tandem Overrides (קוראים את אלה לפני כל השאר)

ה-DESIGN.md הזה מבוסס על שפת העיצוב של Vercel — אבל **רק כשלד למבנה, היררכיה וקצב**.
הצבעים, הטיפוגרפיה והרדיוסים של Tandem **מבטלים** את אלה של Vercel בכל מקום שיש התנגשות.

## Palette (override)
- `--bg-canvas: #f0e9da` (קרם חם — לא לבן/אפור של Vercel)
- `--ink: #18182a` (נייבי כהה לטקסט)
- `--ink-mute: #5a5a6e`
- `--primary: #e85d3a` (טרקוטה — צבע אקצנט יחיד; שכח מ-mesh gradient של Vercel)
- `--primary-press: #c44a2c`
- `--surface: #ffffff` (כרטיסים על קרם)
- `--border: rgba(24, 24, 42, 0.12)`

## Typography (override)
- **Display + Body**: `Rubik` (geometric sans, ידידותית-רצינית)
- **Mono / Meta / Code**: `IBM Plex Mono`
- ❌ אל תשתמש ב-Geist Sans/Mono של Vercel
- כותרות: weight 500–600, letter-spacing נורמלי (לא tight כמו Vercel)

## Geometry (override)
- `--radius: 0.125rem` (חד כמו דפוס — לא ה-`12px`-`16px` הרך של Vercel)
- גבולות 1px בלבד; אין shadows כבדים
- מרווחים נדיבים: scale של 8/16/24/40/64

## Voice
- כותרות בעברית, מונחים טכניים באנגלית (`Mission`, `SPDD`, `RLS`...)
- טון: יועץ בכיר רגוע — לא "פלטפורמת clouds" שיווקית

## מה כן לאמץ מ-Vercel (מתחת לשורה הזו)
- ההיררכיה הטיפוגרפית (display → eyebrow → body → caption)
- הקצב המרחבי וה-section rhythm
- העיקרון של "אקצנט אחד + הרבה שקט"
- מבנה הרכיבים (cards, buttons, nav)
